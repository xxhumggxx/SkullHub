...
local SaveManager = {} do
	...
	SaveManager.HWID = RbxAnalyticsService:GetClientId()
	SaveManager.SaveMode = "Username" -- Default save mode
	...
	function SaveManager:UpdateAutoSaveConfigName()
		if self.SaveMode == "Username" then
			self.AutoSaveConfigName = "AutoSave_" .. self.PlayerName
		else -- HWID mode
			self.AutoSaveConfigName = "AutoSave_" .. self.HWID
		end
	end
	...
	function SaveManager:LoadAutoSaveConfig()
		local autoSaveFile = self.Folder .. "/settings/" .. self.AutoSaveConfigName .. ".json"
		if isfile(autoSaveFile) then
			local success, err = self:Load(self.AutoSaveConfigName)
			if success and self.Library then
				self.Library:Notify({
					Title = "Auto Save",
					Content = "Config loader",
					SubContent = string.format("Auto loaded config for %s", self.SaveMode == "Username" and self.PlayerName or self.HWID),
					Duration = 5
				})
			end
		end
	...
	function SaveManager:BuildConfigSection(tab)
		...
		-- Add Save Mode Dropdown
		local SaveModeDropdown = section:AddDropdown("SaveManager_SaveMode", {
			Title = "Save Mode",
			Description = "Choose how to save configs",
			Values = {"Username", "HWID"},
			Multi = false,
			Default = "Username",
		})
		
		SaveModeDropdown:OnChanged(function(Value)
			self.SaveMode = Value
			self:UpdateAutoSaveConfigName()
			...
			if self.Library then
				self.Library:Notify({
					Title = "Save Mode",
					Content = "Config loader",
					SubContent = string.format("Save mode changed to %s", Value),
					Duration = 3
				})
			end
		end)
			...
			Callback = function(value)
				...
				if value then
					...
					self.Library:Notify({
						Title = "Auto Save",
						Content = "Config loader",
						SubContent = string.format("Auto save enabled for %s", self.SaveMode == "Username" and self.PlayerName or self.HWID),
						Duration = 5
					})
				else
					self:StopAutoSave()
